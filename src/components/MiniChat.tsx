...
export default function MiniChat() {
  ...
  useEffect(() => {
    if (conversationId && currentUser) {
      const unsubMsgs = subscribeMessages(conversationId, (msgs) => {
        setMessages(msgs);
  ...
  const handleSend = async () => {
    if (!inputText.trim() || !conversationId || !currentUser || isSendingRef.current) return;
        ...
        <div className="p-3 bg-white border-b border-gray-100 flex items-center justify-between shadow-sm cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
          <div className="flex items-center space-x-2 min-w-0">
            ...
            <div className="truncate">
              <h4 className="font-normal text-[14px] text-gray-900 truncate">{miniChatUser.name}</h4>
              {isOnline ? (
                <p className="text-[10px] text-green-500 font-normal uppercase tracking-tighter leading-none">Online</p>
              ) : (
                <p className="text-[10px] text-gray-400 font-normal leading-none">Last seen {lastSeen ? formatTime({ toMillis: () => lastSeen } as any) : 'recently'}</p>
              )}
            </div>
          </div>
        ...
        {!isMinimized && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#fafbfc] no-scrollbar">
              {messages.map((msg, idx) => {
                ...
                return (
                  ...
                  >
                    <div 
                      className={`max-w-[80%] px-3 py-2 rounded-2xl text-[13px] font-normal shadow-sm flex items-center space-x-2 ${
                        isMe 
                    ...
                    >
                      {msg.type === 'voice' ? (
                        <div className="flex items-center space-x-2 w-[140px]">
                           ...
                           <span className="text-[10px] font-normal">0:{(msg as any).replyTo?.duration || '00'}</span>
                        </div>
                      ) : (
            ...
            <div className="p-3 bg-white border-t border-gray-100 flex items-center space-x-2 relative overflow-hidden">
              ...
              <div className="flex-1 bg-gray-100 rounded-xl flex items-center px-3 py-1.5 border border-gray-200 focus-within:ring-0 focus-within:outline-none transition-all">
                <input 
                  type="text" 
                  ...
                  className="w-full bg-transparent outline-none focus:outline-none focus:ring-0 text-[13px] font-normal text-gray-800"
                />
              </div>
